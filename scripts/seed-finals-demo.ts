...
// Billing demo — two annotator members whose wallets get credited so a
// fresh seed lands a believable billing state (balances + one pending
// withdrawal). IDs are deterministic so the ledger/wallet upserts below
// stay idempotent across re-runs.
const DEMO_MEMBERS = [
  {
    userId: derivedUuid('member:alice'),
    email: 'finals-demo-alice@labelhub.local',
    displayName: 'Demo Annotator · Alice',
    creditMinor: 5000,
  },
  {
    userId: derivedUuid('member:bob'),
    email: 'finals-demo-bob@labelhub.local',
    displayName: 'Demo Annotator · Bob',
    creditMinor: 8000,
  },
] as const
const DEMO_CURRENCY = 'CNY'
// The member who files the pending withdrawal (Alice) + its amount.
const DEMO_WITHDRAWAL_AMOUNT_MINOR = 3000
...
async function main() {
  ...
  // 7. Billing state — credit two annotator members' wallets via positive
  //    'adjustment' ledger rows + matching wallet_balance snapshots, then
  //    file ONE pending withdrawal. This mirrors the simple credit→withdraw
  //    loop documented on the withdrawal_requests table (admin credits an
  //    account → transactions(type='adjustment', +amount) → wallet rebuilt;
  //    user requests withdrawal → withdrawal_requests(status='requested')).
  //    We insert directly with the db client — no "use server" action — and
  //    keep everything idempotent (deterministic txn ids + onConflictDoNothing,
  //    select-then-upsert on wallet_balance).
  for (const member of DEMO_MEMBERS) {
    // Member user + workspace membership (annotator role).
    await db
      .insert(schema.users)
      .values({
        id: member.userId,
    ...
    // (a) Credit the wallet — append a positive 'adjustment' txn. The txn id
    //     is deterministic so re-running this script never double-credits.
    const txnId = derivedUuid(`txn:adjustment:${member.userId}`)
    await db
      ...
      .values({
        ...
        userId: member.userId,
        type: 'adjustment',
        amountMinor: member.creditMinor, // positive — admin manual credit
        currency: DEMO_CURRENCY,
        workspaceId: DEMO_WORKSPACE_ID,
        memo: 'Finals demo seed — starting balance',
      })
      .onConflictDoNothing()

    // Upsert the matching wallet_balance snapshot for (user × workspace ×
    //   currency). Select-then-insert/update mirrors rebuildWallet() and is
    //   safe to re-run (sets the absolute balance, never accumulates).
    const [wallet] = await db
      .select({ id: schema.walletBalance.id })
      .from(schema.walletBalance)
      .where(
        and(
          eq(schema.walletBalance.userId, member.userId),
          eq(schema.walletBalance.workspaceId, DEMO_WORKSPACE_ID),
          eq(schema.walletBalance.currency, DEMO_CURRENCY),
        ),
      )
      .limit(1)
    if (wallet) {
      await db
        ...
        .set({ balanceMinor: member.creditMinor, lastSettledAt: new Date() })
        .where(eq(schema.walletBalance.id, wallet.id))
    } else {
      await db.insert(schema.walletBalance).values({
        userId: member.userId,
        workspaceId: DEMO_WORKSPACE_ID,
        currency: DEMO_CURRENCY,
        balanceMinor: member.creditMinor,
        lastSettledAt: new Date(),
      })
    }
  }
  console.log(`  ✓ wallets credited (${DEMO_MEMBERS.length} members)`)

  // (b) ONE pending withdrawal for the first member. Deterministic id keeps
  //     this a no-op on re-run; status stays 'requested' (no ledger row yet —
  //     the debit only lands when an admin approves it).
  const withdrawer = DEMO_MEMBERS[0]
  await db
    .insert(schema.withdrawalRequests)
    .values({
      id: derivedUuid(`withdrawal:${withdrawer.userId}`),
      userId: withdrawer.userId,
      workspaceId: DEMO_WORKSPACE_ID,
      amountMinor: DEMO_WITHDRAWAL_AMOUNT_MINOR, // positive request amount
      currency: DEMO_CURRENCY,
      status: 'requested',
    })
    .onConflictDoNothing()
  console.log('  ✓ withdrawal_requests (1 pending)')
