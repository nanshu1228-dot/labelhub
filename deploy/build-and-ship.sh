...
hydrate_public_build_env() {
  if [[ -n "${NEXT_PUBLIC_SUPABASE_URL:-}" && -n "${NEXT_PUBLIC_SUPABASE_ANON_KEY:-}" ]]; then
    return
  fi

  echo "==> loading NEXT_PUBLIC_* build env from server"
  local build_env_tmp
  build_env_tmp="$(mktemp)"
  chmod 600 "$build_env_tmp"
  $SSH "$SSH_TARGET" \
    "awk -F= '/^(NEXT_PUBLIC_SUPABASE_URL|NEXT_PUBLIC_SUPABASE_ANON_KEY|NEXT_PUBLIC_APP_URL|PUBLIC_BASE_URL)=/ { print }' /etc/labelhub.env" \
    > "$build_env_tmp"
  # shellcheck disable=SC1090
  set -a; source "$build_env_tmp"; set +a
  rm -f "$build_env_tmp"

  if [[ -z "${PUBLIC_BASE_URL:-}" && -n "${NEXT_PUBLIC_APP_URL:-}" ]]; then
    export PUBLIC_BASE_URL="$NEXT_PUBLIC_APP_URL"
  fi
  : "${NEXT_PUBLIC_SUPABASE_URL:?NEXT_PUBLIC_SUPABASE_URL missing for browser bundle}"
  : "${NEXT_PUBLIC_SUPABASE_ANON_KEY:?NEXT_PUBLIC_SUPABASE_ANON_KEY missing for browser bundle}"
}
...
if [[ "$mode" != "--restart-only" ]]; then
  ...
  $SSH "$SSH_TARGET" "
    set -e
    chown -R root:root $REMOTE_CODE
    install -d -o labelhub -g labelhub -m 0755 $REMOTE_CODE/.next/cache
  "
fi
