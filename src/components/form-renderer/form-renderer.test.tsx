...
describe('Designer → Renderer round trip (no DOM)', () => {
  ...
  it('runtime current-scope filtering keeps group children for their own value scope', () => {
    ...
    const visibleTop = filterFieldsForCurrentScope(fields, {
      grp: { cat: 'other' },
    })

    expect(visibleTop.map((f) => f.id)).toEqual(['grp'])
    expect(visibleTop[0].children?.map((f) => f.id)).toEqual([
      'cat',
      'details',
    ])
    expect(
      filterFieldsForCurrentScope(fields[0].children ?? [], {
        cat: 'a',
      }).map((f) => f.id),
    ).toEqual(['cat'])
    expect(
  ...
  it('AI assist detection walks visible group and tab scopes recursively', () => {
    const fields: FieldNode[] = [
      {
        id: 'tabs',
        kind: 'tab-layout',
