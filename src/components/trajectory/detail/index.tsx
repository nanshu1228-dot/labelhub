...
import type {
  stepAnnotations as stepAnnotationsTable,
  trajectorySteps,
} from '@/lib/db/schema'
...
} from '@/lib/trajectories/meta-display'

/**
 * Presentation for the read-only trajectory inspector
 * (`/workspaces/[id]/trajectories/[trajId]`).
 *
 * Extracted out of the route's `page.tsx` so that file stays a thin
 * data-fetch + compose shell. Everything here is server-renderable (no
 * hooks) — the lone interactive piece is `StepMarkWidget`, a client
 * component rendered as a leaf. The page composes the four exported
 * pieces (`Header`, `Body`, `ReviewModeBanner`, `DbError`); the rest are
 * module-private building blocks. Pure formatters live in
...
// Error state
//
// Re-exported from the shared panel so the route's barrel import
// (`@/components/trajectory/detail`) keeps working. The per-surface wording
// ("Couldn't load this trajectory.") that the old local copy hard-coded now
// lives at the call site as the `description` prop.
