...
/**
 * GET /api/export/dataset?versionId=...&format=raw|teaching&encoding=json|jsonl|csv|excel&mapping=[...]
 *
 ...
 *
 *   `mapping` (optional)
 *     - JSON array of { source, target, transform? } entries. Source
 *       uses dotted paths into each row; target is the output field /
 *       column. `transform=json_stringify` keeps nested values usable
 *       in CSV / Excel cells.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const mapping = parseFieldMappingParam(url.searchParams.get('mapping'))
    const formatOptions: FormatOptions | undefined = mapping
      ? { mapping }
      : undefined
    if (!versionId) {
      throw new AppError(
    ...
    // M1 — per-format pre-flight validation. Fail fast with a typed
    // ValidationError (which row/field) before any bytes are written
    // or a job is enqueued, so a malformed payload never produces a
    // corrupt/truncated file. Honors the same field mapping the
    // formatter applies, so flattened cells aren't falsely rejected.
    validateRowsForFormat(entries, encoding, { mapping })
    ...
    if (shouldGoAsync) {
      ...
      after(async () => {
        ...
        try {
          ...
          let totalBytes2 = 0
          for await (const chunk of formatter(rowIterator(), formatOptions)) {
            chunks2.push(chunk)
