...
describe('formatCSV', () => {
  ...
  it('exports a negative number verbatim (no formula-injection guard)', async () => {
    const text = await asString(
      formatCSV(rowGen([{ id: '1', delta: -5 }])),
    )
    const lines = text.trim().split('\n')
    // The numeric -5 must serialize as -5, NOT be mangled into "'-5".
    expect(lines[0]).toBe('id,delta')
    expect(lines[1]).toBe('1,-5')
    expect(text).not.toContain(`'-5`)
  })

  it('still guards STRING cells beginning with = + @ or -', async () => {
    const text = await asString(
      formatCSV(
        rowGen([
          { eq: '=A1', plus: '+B1', at: '@cmd', dash: '-evil' },
        ]),
      ),
    )
    ...
    // A string starting with `-` is still escaped (only NUMERIC -5 is exempt).
