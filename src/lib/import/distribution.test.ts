...
describe('distributeTopics — open-queue', () => {
  it('leaves all rows unassigned for first-come claiming', () => {
    const a = distributeTopics('open-queue', {
      topicCount: 4,
      annotators: ANNOTATORS,
    })
    expect(a).toEqual([
      { topicIndex: 0, annotatorId: null },
      { topicIndex: 1, annotatorId: null },
      { topicIndex: 2, annotatorId: null },
      { topicIndex: 3, annotatorId: null },
    ])
  })
})
