 ...
 * consumes. Materials (text / textarea / select / tag-select / file / json-editor /
...
export const CUSTOM_VALIDATION_FUNCTIONS = [
  'contains',
  'not-contains',
  'starts-with',
  'ends-with',
...
export type CustomValidationFunction =
  (typeof CUSTOM_VALIDATION_FUNCTIONS)[number]
...
export const validationRuleSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    kind: z.literal('custom-function'),
    functionName: z.enum(CUSTOM_VALIDATION_FUNCTIONS),
    argument: z.string().max(500).optional(),
