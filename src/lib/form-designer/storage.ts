...
/**
 * Read one saved schema. By default archived rows stay hidden from
 * admin edit / task creation flows; runtime callers can set
 * includeArchived=true so tasks already pinned to an old schema keep
 * rendering after the owner cleans up the Designer library.
 */
...
> {
  ...
  return rows.map((r) => {
    ...
    return {
      ...
      hasLlmTrigger: hasKind(fields, 'llm-trigger'),
...
function countFields(fields: FieldNode[]): number {
  ...
  for (const field of fields) {
    total += 1
    if (field.children) total += countFields(field.children)
  }
  return total
}

function hasKind(fields: FieldNode[], kind: FieldNode['kind']): boolean {
  for (const field of fields) {
    if (field.kind === kind) return true
    if (field.children && hasKind(field.children, kind)) return true
  }
  return false
}
