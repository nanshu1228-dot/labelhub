...
/**
 * For a `single-select` / `multi-select` / closed `tag-select` field, lift `options[].value`
...
function applyEnumIfSelect(target: JSONSchemaField, field: FieldNode): void {
  ...
  if (field.kind === 'single-select') {
    ...
    (field.kind === 'tag-select' && cfg.allowCustom === false)
  ) {
 ...
 *   - file-upload: cfg.maxFiles → maxItems; items accept legacy URLs
 *     or uploaded file metadata objects
 *   - multi-select: cfg.minSelected / cfg.maxSelected → minItems / maxItems
 *   - tag-select: cfg.minTags / cfg.maxTags → minItems / maxItems
 */
function applyConfigLimits(target: JSONSchemaField, field: FieldNode): void {
  ...
  if (field.kind === 'tag-select') {
    if (typeof cfg.minTags === 'number' && target.minItems === undefined) {
      target.minItems = cfg.minTags
    }
    if (typeof cfg.maxTags === 'number' && target.maxItems === undefined) {
      target.maxItems = cfg.maxTags
    }
  }
}
