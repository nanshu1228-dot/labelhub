...
describe('FormSchema → JSON Schema → FormSchema (round-trip)', () => {
  ...
  it('round-trips a tag-select with custom tags and max item limit', () => {
    const schema: FormSchema = {
      version: FORM_SCHEMA_VERSION,
      fields: [
        mkField('routing_tags', {
          kind: 'tag-select',
          label: 'Routing tags',
          config: {
            options: [
    ...
    const doc = toJsonSchema(schema)
    expect(doc.properties.routing_tags.type).toBe('array')
    expect(doc.properties.routing_tags.maxItems).toBe(5)
    expect(doc.properties.routing_tags.items).toBeUndefined()
    expectRoundTrip(schema)
  })

  it('closed tag-select lifts configured tags into items.enum', () => {
    const schema: FormSchema = {
      version: FORM_SCHEMA_VERSION,
      fields: [
        mkField('review_tags', {
    ...
    const doc = toJsonSchema(schema)
    expect(doc.properties.review_tags.items).toEqual({
      type: 'string',
      enum: ['safe', 'unsafe'],
    })
    expect(doc.properties.review_tags.uniqueItems).toBe(true)
    expectRoundTrip(schema)
  })
  ...
  it('round-trips a file-upload field with accept + size + count caps', () => {
    ...
    expect(doc.properties.avatar.items).toEqual({
      oneOf: [
        { type: 'string' },
        {
          type: 'object',
          required: ['url', 'path', 'name', 'size'],
          properties: {
            url: { type: 'string' },
            path: { type: 'string' },
            name: { type: 'string' },
  ...
  it('round-trips custom-function validation in labelhub metadata', () => {
    const schema: FormSchema = {
      version: FORM_SCHEMA_VERSION,
      fields: [
        mkField('ticket', {
          kind: 'text',
          label: 'Ticket',
          config: { placeholder: 'LH-123' },
          validation: [
            {
              kind: 'custom-function',
              functionName: 'starts-with',
              argument: 'LH-',
              message: 'Use the LH ticket prefix.',
            },
          ],
        }),
      ],
    ...
    const doc = toJsonSchema(schema)
    expect(doc.properties.ticket['x-labelhub-validation']).toEqual(
      schema.fields[0].validation,
    )
    expectRoundTrip(schema)
  })
