...
function normalizeLocalKey(key: string): string | null {
  const safeKey = key.replace(/\\/g, '/').replace(/^\/+/, '')
  return safeKey.includes('..') ? null : safeKey
}

function normalizeLocalBucket(bucket: string): string {
  return bucket.replace(/[^a-zA-Z0-9_-]/g, '_')
}

export function getLocalFsPublicUrl(opts: {
  bucket: string
  key: string
}): string | null {
  const { baseUrl } = getLocalConfig()
  const safeKey = normalizeLocalKey(opts.key)
  if (!safeKey) return null
  const safeBucket = normalizeLocalBucket(opts.bucket)
  return `${baseUrl}/${safeBucket}/${safeKey}`
}
...
export async function uploadToLocalFs(
  ...
  const safeKey = normalizeLocalKey(input.key)
  if (!safeKey) {
    throw new Error(`unsafe storage key: ${input.key}`)
  }
  const safeBucket = normalizeLocalBucket(input.bucket)
  ...
  return {
    publicUrl: getLocalFsPublicUrl({ bucket: input.bucket, key: safeKey })!,
    path: safeKey,
  }
}
