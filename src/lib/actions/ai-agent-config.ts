...
export async function getAiAgentConfig(input: {
  ...
  // The rubric-judgment template mode has its own meta-review preset (rubric
  // quality + judgement correctness); seed the form with it so the owner sees
  // the intended two-dimension rubric instead of the generic default.
  if (task.templateMode === 'rubric-judgment') {
    return {
      workspaceId: task.workspaceId,
      templateMode: task.templateMode,
      config: { ...DEFAULT_RUBRIC_JUDGMENT_CONFIG },
    }
  }
