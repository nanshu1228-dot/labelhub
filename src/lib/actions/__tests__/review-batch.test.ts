...
describe('batchReviewAnnotations — validation', () => {
  ...
  it('requires feedback for batch request_revision before dispatch', async () => {
    await expect(
      batchReviewAnnotations({
        annotationIds: IDS,
    ...
    expect(vi.mocked(qcReviewAnnotation)).not.toHaveBeenCalled()
  })
})
...
describe('batchReviewAnnotations — dispatch', () => {
  ...
  it('approves awaiting_acceptance rows through admin terminal acceptance', async () => {
    vi.mocked(reviewAnnotation).mockResolvedValue({ ok: true } as never)
    vi.mocked(qcReviewAnnotation).mockResolvedValue({
      ok: true,
      next: 'awaiting_acceptance',
    } as never)
    ...
    expect(result.succeeded).toEqual(IDS)
    expect(vi.mocked(reviewAnnotation)).toHaveBeenCalledTimes(1)
    expect(vi.mocked(reviewAnnotation)).toHaveBeenCalledWith({
      annotationId: IDS[0],
      decision: 'approve',
      feedback: undefined,
    })
    expect(vi.mocked(qcReviewAnnotation)).toHaveBeenCalledTimes(2)
    expect(vi.mocked(qcReviewAnnotation).mock.calls.map((c) => c[0].annotationId)).toEqual([
      IDS[1],
      IDS[2],
    ])
  })
